-- 1. Thông tin Người dùng (User)
CREATE TABLE APP_USER (
    USER_ID      NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    FULL_NAME    VARCHAR2(100) NOT NULL,
    PHONE_NUMBER VARCHAR2(20) UNIQUE NOT NULL,
    EMAIL        VARCHAR2(255) UNIQUE NOT NULL,
    ADDRESS      VARCHAR2(255),
    BIRTH        DATE,
    GENDER       VARCHAR2(10) ,
    USER_TYPE    VARCHAR2(20) DEFAULT 'CUSTOMER' ,
    CREATED_AT   DATE DEFAULT SYSDATE,
    UPDATED_AT   DATE DEFAULT SYSDATE,
    IS_DELETED   NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1)),
    CONSTRAINT CHK_GENDER_SCOPE CHECK (GENDER IN ('Nam', 'Nữ')),
    CONSTRAINT CHK_USERTYPE_SCOPE CHECK (USER_TYPE IN ('ADMIN', 'STAFF', 'CUSTOMER'))
);
-- 2. Tài khoản đăng nhập
CREATE TABLE ACCOUNT (
    ACCOUNT_ID    NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    USER_ID       NUMBER(10) NOT NULL,
    USERNAME      VARCHAR2(50) UNIQUE NOT NULL,
    PASSWORD_HASH VARCHAR2(200) NOT NULL,
    STATUS        NUMBER(1) DEFAULT 1 CHECK (STATUS IN (0,1)),
    CREATED_AT    DATE DEFAULT SYSDATE,
    UPDATED_AT    DATE DEFAULT SYSDATE,
    IS_DELETED    NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 3. Phân hệ chức năng
CREATE TABLE FUNCTION (
    FUNCTION_ID   NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    NAME_FUNCTION VARCHAR2(100) NOT NULL,
    CREATED_AT    DATE DEFAULT SYSDATE,
    UPDATED_AT    DATE DEFAULT SYSDATE,
    IS_DELETED    NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 4. Quyền chi tiết
CREATE TABLE ROLE (
    ROLE_ID       NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    FUNCTION_ID   NUMBER(10) NOT NULL,
    ROLE_NAME     VARCHAR2(100) NOT NULL,
    ADD_PERM      NUMBER(1) DEFAULT 0 CHECK (ADD_PERM IN (0,1)),
    EDIT_PERM     NUMBER(1) DEFAULT 0 CHECK (EDIT_PERM IN (0,1)),
    DELETE_PERM   NUMBER(1) DEFAULT 0 CHECK (DELETE_PERM IN (0,1)),
    VIEW_PERM     NUMBER(1) DEFAULT 0 CHECK (VIEW_PERM IN (0,1)),
    DOWNLOAD_PERM NUMBER(1) DEFAULT 0 CHECK (DOWNLOAD_PERM IN (0,1)),
    CREATED_AT    DATE DEFAULT SYSDATE,
    UPDATED_AT    DATE DEFAULT SYSDATE,
    IS_DELETED    NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 5. Nhóm quyền
CREATE TABLE ROLE_GROUP (
    ROLE_GROUP_ID   NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    NAME_ROLE_GROUP VARCHAR2(100) NOT NULL,
    CREATED_AT      DATE DEFAULT SYSDATE,
    UPDATED_AT      DATE DEFAULT SYSDATE,
    IS_DELETED      NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 6. Gán Quyền vào Nhóm
CREATE TABLE ROLE_GROUP_ASSIGN_ROLE (
    ROLE_GROUP_ID NUMBER(10) NOT NULL,
    ROLE_ID       NUMBER(10) NOT NULL,
    CREATED_AT      DATE DEFAULT SYSDATE,
    UPDATED_AT      DATE DEFAULT SYSDATE,
    IS_DELETED      NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1)),
    PRIMARY KEY (ROLE_GROUP_ID, ROLE_ID)
);

-- 7. Gán Tài khoản vào Nhóm
CREATE TABLE ACCOUNT_ASSIGN_ROLE_GROUP (
    ACCOUNT_ID    NUMBER(10) NOT NULL,
    ROLE_GROUP_ID NUMBER(10) NOT NULL,
    CREATED_AT      DATE DEFAULT SYSDATE,
    UPDATED_AT      DATE DEFAULT SYSDATE,
    IS_DELETED      NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1)),
    PRIMARY KEY (ACCOUNT_ID, ROLE_GROUP_ID)
);

-- 8. Gán Quyền trực tiếp cho Tài khoản
CREATE TABLE ACCOUNT_ASSIGN_ROLE (
    ACCOUNT_ID NUMBER(10) NOT NULL,
    ROLE_ID    NUMBER(10) NOT NULL,
    CREATED_AT      DATE DEFAULT SYSDATE,
    UPDATED_AT      DATE DEFAULT SYSDATE,
    IS_DELETED      NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1)),
    PRIMARY KEY (ACCOUNT_ID, ROLE_ID)
);

-- 9. Token phiên làm việc
CREATE TABLE ACCOUNT_TOKEN (
    TOKEN_ID      NUMBER(20) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ACCOUNT_ID    NUMBER(10) NOT NULL,
    TOKEN_VALUE   VARCHAR2(500) UNIQUE NOT NULL,
    EXPIRES_AT    DATE NOT NULL,
    ISSUED_AT     DATE DEFAULT SYSDATE,
    IS_REVOKED    CHAR(1) DEFAULT 'N' CHECK (IS_REVOKED IN ('Y', 'N'))
);

-- 10. Danh mục sản phẩm
CREATE TABLE CATEGORY (
    CATEGORY_ID        NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    NAME               VARCHAR2(40) NOT NULL,
    CREATED_AT         DATE DEFAULT SYSDATE,
    IS_DELETED         NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 11. Sản phẩm
CREATE TABLE PRODUCT (
    PRODUCT_ID     NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    NAME           VARCHAR2(100) NOT NULL,
    IMAGE_NAME     VARCHAR2(255) NOT NULL,
    DESCRIPTION    VARCHAR2(255),
    PRICE          NUMBER(18,2) NOT NULL CHECK (PRICE >= 0),
    STOCK_QUANTITY NUMBER(10) DEFAULT 0 CHECK (STOCK_QUANTITY >= 0),
    STATUS         NUMBER(1) DEFAULT 1 CHECK (STATUS IN (0,1)),
    WARRANTY_MONTH NUMBER(3) NOT NULL,
    CATEGORY_ID    NUMBER(10) NOT NULL,
    BRAND_ID    NUMBER(10) NOT NULL,
    CREATED_AT     DATE DEFAULT SYSDATE,
    UPDATED_AT     DATE DEFAULT SYSDATE,
    IS_DELETED         NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);


CREATE TABLE BRAND (
    BRAND_ID           NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    NAME               VARCHAR2(50) NOT NULL,
    CREATED_AT         DATE DEFAULT SYSDATE,
    IS_DELETED         NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);
-- 12. Đối tác Affiliate
CREATE TABLE AFFILIATE (
    AFFILIATE_ID    NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    USER_ID         NUMBER(10) NOT NULL,
    COMMISSION_RATE NUMBER(5,2) DEFAULT 5.0,
    TOTAL_EARNED    NUMBER(18,2) DEFAULT 0,
    STATUS          NUMBER(1) DEFAULT 1 CHECK (STATUS IN (0,1)),
    CREATED_AT      DATE DEFAULT SYSDATE,
    UPDATED_AT      DATE DEFAULT SYSDATE,
    IS_DELETED      NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 13. Mã giảm giá (Coupon)
CREATE TABLE COUPON (
    COUPON_ID        NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    CODE             NVARCHAR2(30) NOT NULL UNIQUE,
    DISCOUNT_TYPE    NVARCHAR2(10) NOT NULL CHECK (DISCOUNT_TYPE IN ('PERCENTAGE', 'AMOUNT')),
    DISCOUNT_VALUE   DECIMAL(18,2) NOT NULL CHECK (DISCOUNT_VALUE >= 0),
    MIN_ORDER_VALUE  DECIMAL(18,2) DEFAULT 0 CHECK (MIN_ORDER_VALUE >= 0),
    MAX_DISCOUNT     DECIMAL(18,2) CHECK (MAX_DISCOUNT >= 0),
    START_AT         DATE NOT NULL,
    END_AT           DATE NOT NULL,
    IS_ACTIVE        NUMBER(1) DEFAULT 1 CHECK (IS_ACTIVE IN (0,1)),
    DESCRIPTION      NVARCHAR2(255),
    AFFILIATE_ID     NUMBER(10),
    CREATED_AT       DATE DEFAULT SYSDATE,
    UPDATED_AT       DATE DEFAULT SYSDATE,
    IS_DELETED       NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1)),
    CONSTRAINT CHK_COUPON_DATES CHECK (END_AT > START_AT)
);


-- 16. Đơn hàng
CREATE TABLE ORDERS (
    ORDER_ID         NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    USER_ID          NUMBER(10) NOT NULL,
    COUPON_ID        NUMBER,
    SUBTOTAL         NUMBER(18,2) DEFAULT 0 NOT NULL, --Chưa ship, chưa giảm giá
    DISCOUNT_AMOUNT  NUMBER(18,2) DEFAULT 0,
    SHIPPING_FEE     NUMBER(18,2) DEFAULT 0,
    TOTAL            NUMBER(18,2) DEFAULT 0 NOT NULL, --Cuối cùng
    STATUS           VARCHAR2(30) NOT NULL,
    RECEIVER_NAME    VARCHAR2(100) NOT NULL,
    RECEIVER_PHONE   VARCHAR2(20) NOT NULL,
    SHIPPING_ADDRESS NVARCHAR2(500) NOT NULL,
    CREATED_AT       DATE DEFAULT SYSDATE,
    UPDATED_AT       DATE DEFAULT SYSDATE,
    IS_DELETED       NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1)),
    CONSTRAINT CHK_ORDER_STATUS_SCOPE  CHECK (STATUS IN ('PENDING','CONFIRMED','PROCESSING','SHIPPING','DELIVERED','CANCELLED'))
);

-- 17. Chi tiết đơn hàng
CREATE TABLE ORDER_DETAIL (
    ORDER_DETAIL_ID NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ORDER_ID        NUMBER(10) NOT NULL,
    PRODUCT_ID      NUMBER(10) NOT NULL,
    QUANTITY        NUMBER(10) NOT NULL CHECK (QUANTITY > 0),
    PRICE           NUMBER(18,2) NOT NULL,
    LINE_TOTAL      NUMBER(18,2) DEFAULT 0 --quantity * price
);

-- 18. Thanh toán
CREATE TABLE PAYMENT (
    PAYMENT_ID       NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ORDER_ID         NUMBER(10) NOT NULL,
    PAYMENT_METHOD   VARCHAR2(30) NOT NULL,
    AMOUNT           NUMBER(18,2) NOT NULL,
    PAYMENT_STATUS   VARCHAR2(20) DEFAULT 'PENDING',
    PAID_AT          DATE,
    CREATED_AT       DATE DEFAULT SYSDATE,
    UPDATED_AT       DATE DEFAULT SYSDATE
);

-- 19. Chiến dịch khuyến mãi
CREATE TABLE PROMOTION (
    PROMOTION_ID   NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    PROMOTION_NAME VARCHAR2(120) NOT NULL,
    START_AT       DATE NOT NULL,
    END_AT         DATE NOT NULL,
    IS_ACTIVE      NUMBER(1) DEFAULT 1,
    CREATED_AT     DATE DEFAULT SYSDATE,
    UPDATED_AT     DATE DEFAULT SYSDATE,
    IS_DELETED     NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 20. Sản phẩm thuộc khuyến mãi
CREATE TABLE PROMOTION_PRODUCT (
    PROMOTION_ID   NUMBER(10) NOT NULL,
    PRODUCT_ID     NUMBER(10) NOT NULL,
    DISCOUNT_VALUE NUMBER(18,2),
    PRIMARY KEY (PROMOTION_ID, PRODUCT_ID)
);

-- 21. Đánh giá
CREATE TABLE REVIEW (
    REVIEW_ID   NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    USER_ID     NUMBER(10) NOT NULL,
    PRODUCT_ID  NUMBER(10) NOT NULL,
    RATING      NUMBER(1) CHECK (RATING BETWEEN 1 AND 5),
    CONTENT     VARCHAR2(500),
    CREATED_AT     DATE DEFAULT SYSDATE,
    UPDATED_AT     DATE DEFAULT SYSDATE,
    IS_DELETED     NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 22. Giỏ hàng
CREATE TABLE CART (
    CART_ID    NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    USER_ID    NUMBER(10) NOT NULL,
    CREATED_AT DATE DEFAULT SYSDATE,
    UPDATED_AT     DATE DEFAULT SYSDATE,
    IS_DELETED     NUMBER(1) DEFAULT 0 CHECK (IS_DELETED IN (0,1))
);

-- 23. Chi tiết giỏ hàng
CREATE TABLE CART_ITEM (
    CART_ITEM_ID NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    CART_ID      NUMBER(10) NOT NULL,
    PRODUCT_ID   NUMBER(10) NOT NULL,
    QUANTITY     NUMBER(10) NOT NULL CHECK (QUANTITY > 0),
    TOTAL        NUMBER(18,2) NOT NULL CHECK (TOTAL > 0),
    CONSTRAINT UQ_CART_PRODUCT UNIQUE (CART_ID, PRODUCT_ID)
);
-- 24. Điểm tích lũy
CREATE TABLE LOYALTY_POINT (
    USER_ID      NUMBER(10) NOT NULL PRIMARY KEY,
    TOTAL_POINTS NUMBER(10) DEFAULT 0,
    UPDATED_AT   DATE DEFAULT SYSDATE
);
--INSERT DỮ LIỆU
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Dashboard');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Thống kê');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Doanh số');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Bán hàng tại quầy');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Quản lý sản phẩm');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Quản lý danh mục');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Quản lý kho hàng');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Quản lý đơn hàng');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Quản lý khuyến mãi');
INSERT INTO FUNCTION (NAME_FUNCTION) VALUES ('Quản lý tài khoản');
commit;
--Category
INSERT INTO CATEGORY (NAME) VALUES (N'Laptop');
INSERT INTO CATEGORY (NAME) VALUES (N'Điện thoại');
INSERT INTO CATEGORY (NAME) VALUES (N'Tai nghe');
INSERT INTO CATEGORY (NAME) VALUES (N'Màn hình');
INSERT INTO CATEGORY (NAME) VALUES (N'Đồng hồ thông minh');
INSERT INTO CATEGORY (NAME) VALUES (N'Máy chơi game');
INSERT INTO CATEGORY (NAME) VALUES (N'Phụ kiện'); select * from CATEGORY;
commit;
--Promotion & Promotion Product
INSERT INTO PROMOTION (PROMOTION_NAME, START_AT, END_AT, IS_ACTIVE, CREATED_AT, UPDATED_AT, IS_DELETED)
VALUES (
    'Back to School 2026_2',
    TO_DATE('2026-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS'),
    TO_DATE('2026-09-30 23:59:59', 'YYYY-MM-DD HH24:MI:SS'),
    1,
    SYSDATE,
    SYSDATE,
    0
);
select * from PROMOTION;

INSERT INTO PROMOTION_PRODUCT (PROMOTION_ID, PRODUCT_ID, DISCOUNT_VALUE)
VALUES (1, 2, 16000000);
commit;
INSERT INTO PROMOTION_PRODUCT (PROMOTION_ID, PRODUCT_ID, DISCOUNT_VALUE)
VALUES ((SELECT PROMOTION_ID FROM PROMOTION WHERE PROMOTION_NAME = 'Back to School 2026_2' AND IS_DELETED = 0), 2, 1500000);

INSERT INTO PROMOTION_PRODUCT (PROMOTION_ID, PRODUCT_ID, DISCOUNT_VALUE)
VALUES ((SELECT PROMOTION_ID FROM PROMOTION WHERE PROMOTION_NAME = 'Back to School 2026_2' AND IS_DELETED = 0), 3, 2000000);

COMMIT;

--UNIQUE INDEX dành cho is_deleted
CREATE UNIQUE INDEX UIX_CATEGORY_NAME_ACTIVE
ON CATEGORY (
    CASE WHEN IS_DELETED = 0 THEN NAME ELSE NULL END
);
CREATE UNIQUE INDEX UIX_BRAND_NAME_ACTIVE
ON BRAND (
    CASE WHEN IS_DELETED = 0 THEN NAME ELSE NULL END
);

CREATE UNIQUE INDEX UIX_ROLE_GROUP_NAME_ACTIVE
ON ROLE_GROUP (
    CASE WHEN IS_DELETED = 0 THEN NAME_ROLE_GROUP ELSE NULL END
);



--Testing
-- ===========================================================================
-- 1. BRAND & CATEGORY
-- ===========================================================================
INSERT INTO BRAND (NAME) VALUES ('Apple');
INSERT INTO BRAND (NAME) VALUES ('Samsung');
INSERT INTO BRAND (NAME) VALUES ('Sony');

INSERT INTO CATEGORY (NAME) VALUES ('Điện thoại');
INSERT INTO CATEGORY (NAME) VALUES ('Laptop');
INSERT INTO CATEGORY (NAME) VALUES ('Phụ kiện');

COMMIT;

-- ===========================================================================
-- 2. SẢN PHẨM (PRODUCT)
-- Giả sử BRAND_ID: Apple=1, Samsung=2, Sony=3
--         CATEGORY_ID: Điện thoại=1, Laptop=2, Phụ kiện=3
-- ===========================================================================
INSERT INTO PRODUCT (NAME, IMAGE_NAME, DESCRIPTION, PRICE, STOCK_QUANTITY, WARRANTY_MONTH, CATEGORY_ID, BRAND_ID)
VALUES ('iPhone 15 Pro', 'iphone15pro.jpg', 'Chip A17 Pro, camera 48MP', 28000000, 50, 12, 1, 1);

INSERT INTO PRODUCT (NAME, IMAGE_NAME, DESCRIPTION, PRICE, STOCK_QUANTITY, WARRANTY_MONTH, CATEGORY_ID, BRAND_ID)
VALUES ('Samsung Galaxy S24', 'galaxys24.jpg', 'Chip Snapdragon 8 Gen 3', 22000000, 30, 12, 1, 2);

INSERT INTO PRODUCT (NAME, IMAGE_NAME, DESCRIPTION, PRICE, STOCK_QUANTITY, WARRANTY_MONTH, CATEGORY_ID, BRAND_ID)
VALUES ('MacBook Pro M3', 'macbookm3.jpg', 'Chip M3, RAM 16GB', 45000000, 20, 12, 2, 1);

INSERT INTO PRODUCT (NAME, IMAGE_NAME, DESCRIPTION, PRICE, STOCK_QUANTITY, WARRANTY_MONTH, CATEGORY_ID, BRAND_ID)
VALUES ('Tai nghe Sony WH-1000XM5', 'sony-wh1000xm5.jpg', 'Chống ồn hàng đầu', 8000000, 100, 12, 3, 3);

COMMIT;

-- ===========================================================================
-- 3. USERS
--    User 1: Khách hàng mới (chưa có điểm tích lũy)
--    User 2: Khách hàng thân thiết (có lịch sử mua hàng)
--    User 3: Dùng để test fraud detection
-- ===========================================================================
INSERT INTO APP_USER (FULL_NAME, PHONE_NUMBER, EMAIL, ADDRESS, GENDER, USER_TYPE)
VALUES ('Nguyễn Văn An', '0901111111', 'an@email.com', 'Quận 1, HCM', 'Nam', 'CUSTOMER');

INSERT INTO APP_USER (FULL_NAME, PHONE_NUMBER, EMAIL, ADDRESS, GENDER, USER_TYPE)
VALUES ('Trần Thị Bình', '0902222222', 'binh@email.com', 'Quận 3, HCM', 'Nữ', 'CUSTOMER');

INSERT INTO APP_USER (FULL_NAME, PHONE_NUMBER, EMAIL, ADDRESS, GENDER, USER_TYPE)
VALUES ('Lê Văn Cường', '0903333333', 'cuong@email.com', 'Quận 7, HCM', 'Nam', 'CUSTOMER');

COMMIT;

-- ===========================================================================
-- 4. KHUYẾN MÃI (để test fn_get_product_effective_price)
--    Tạo promotion giảm 2.000.000đ cho iPhone 15 Pro
-- ===========================================================================
INSERT INTO PROMOTION (PROMOTION_NAME, START_AT, END_AT, IS_ACTIVE)
VALUES ('Sale Tháng 5', SYSDATE - 1, SYSDATE + 30, 1);

-- Giả sử PROMOTION_ID = 1, PRODUCT_ID iPhone = 1
INSERT INTO PROMOTION_PRODUCT (PROMOTION_ID, PRODUCT_ID, DISCOUNT_VALUE)
VALUES (1, 1, 2000000);

COMMIT;

-- ===========================================================================
-- 5. ĐIỂM TÍCH LŨY BAN ĐẦU cho User 2 (để test hệ số nhân hạng BẠC)
--    Gán sẵn 600 điểm → hạng BẠC → hệ số x1.5
-- ===========================================================================
INSERT INTO LOYALTY_POINT (USER_ID, TOTAL_POINTS, UPDATED_AT)
VALUES (2, 600, SYSDATE);

COMMIT;

-- Tạo đơn hàng cho User 1
INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (1, 5000000, 5000000, 'PENDING', 'Nguyễn Văn An', '0901111111', 'Quận 1, HCM');

INSERT INTO ORDER_DETAIL (ORDER_ID, PRODUCT_ID, QUANTITY, PRICE)
VALUES (1, 4, 1, 5000000);   -- Tai nghe Sony

-- Tạo đơn hàng cho User 2
INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (2, 5000000, 5000000, 'PENDING', 'Trần Thị Bình', '0902222222', 'Quận 3, HCM');

INSERT INTO ORDER_DETAIL (ORDER_ID, PRODUCT_ID, QUANTITY, PRICE)
VALUES (2, 4, 1, 5000000);

COMMIT;

-- Chuyển cả 2 đơn sang DELIVERED → trigger tự động cộng điểm
UPDATE ORDERS SET STATUS = 'DELIVERED' WHERE ORDER_ID = 1;
UPDATE ORDERS SET STATUS = 'DELIVERED' WHERE ORDER_ID = 2;
COMMIT;


-- Tạo đơn hàng đang CONFIRMED chứa MacBook Pro (PRODUCT_ID = 3)
INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (1, 45000000, 45000000, 'CONFIRMED', 'Nguyễn Văn An', '0901111111', 'Quận 1, HCM');

INSERT INTO ORDER_DETAIL (ORDER_ID, PRODUCT_ID, QUANTITY, PRICE)
VALUES (3, 3, 1, 45000000);   -- MacBook Pro

COMMIT;



-- CASE A: Spam 5 đơn hàng trong vài giây
INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (3, 1000000, 1000000, 'PENDING', 'Lê Văn Cường', '0903333333', 'Quận 7, HCM');

INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (3, 1000000, 1000000, 'PENDING', 'Lê Văn Cường', '0903333333', 'Quận 7, HCM');

INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (3, 1000000, 1000000, 'PENDING', 'Lê Văn Cường', '0903333333', 'Quận 7, HCM');

INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (3, 1000000, 1000000, 'PENDING', 'Lê Văn Cường', '0903333333', 'Quận 7, HCM');

INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
VALUES (3, 1000000, 1000000, 'PENDING', 'Lê Văn Cường', '0903333333', 'Quận 7, HCM');

COMMIT;




BEGIN
    INSERT INTO ORDERS (USER_ID, SUBTOTAL, TOTAL, STATUS, RECEIVER_NAME, RECEIVER_PHONE, SHIPPING_ADDRESS)
    VALUES (3, 1000000, 1000000, 'PENDING', 'Lê Văn Cường', '0903333333', 'Quận 7, HCM');
    DBMS_OUTPUT.PUT_LINE('>>> INSERT thành công (không bị chặn)');
    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('>>> BỊ CHẶN đúng như kỳ vọng: ' || SQLERRM);
        ROLLBACK;
END;
/

-- --xóa sequence
-- BEGIN
--     FOR s IN (SELECT sequence_name FROM user_sequences) LOOP
--         BEGIN
--             EXECUTE IMMEDIATE 'DROP SEQUENCE ' || s.sequence_name;
--         EXCEPTION
--             WHEN OTHERS THEN
--                 -- Nếu đụng phải sequence hệ thống cứng đầu, đoạn này sẽ ép xóa luôn
--                 NULL;
--         END;
--     END LOOP;
-- END;
-- /
--
-- -- Xóa triệt để cả thùng rác hệ thống một lần nữa cho chắc chắn
-- PURGE RECYCLEBIN;






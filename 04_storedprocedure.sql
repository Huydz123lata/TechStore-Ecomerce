...
BEGIN
   ...
   IF v_current_status IN ('SHIPPING', 'DELIVERED', 'CANCELLED') THEN
       DBMS_OUTPUT.PUT_LINE('Cannot cancel an order that is shipping, delivered, or already cancelled.');
   ELSE
...
BEGIN
    ...
    ELSE
        ...
        VALUES (p_code, 'AMOUNT', p_discount_value, SYSDATE, SYSDATE + p_valid_days, p_limit);

        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Coupon created successfully. Valid for ' || p_valid_days || ' days.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
